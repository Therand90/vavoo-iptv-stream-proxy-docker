...
replaceExactlyOnce(
  ...
  `function getLocalChannelPlaylistUrl(req, channelId) {
    return \`${'${req.protocol}'}://${'${req.headers.host}'}/hls-channel/${'${encodeURIComponent(channelId)}'}\`;
}
...
async function fetchChannelPlaylist(req, channel, forceRefresh = false) {
    ...
    if (!isM3u8Response(streamUrl, contentType)) {
        throw new Error(\`upstream did not return an HLS playlist (${ '${contentType || \'unknown\'}' })\`);
    }
  ...
  `app.get('/hls-channel/:id', async function (req, res) {
    ...
    try {
        ...
        console.log(
            ...
            \`status=${'${upstream.status}'} sequence=${'${debugInfo.sequence}'} ` +
