...
if (!target) {
  console.error('Usage: node rank-logical-variants.mjs <index.js>');
  process.exit(1);
}
...
function scoreLogicalVariantQuality(result, variant) {
    ...
    // The suffix is only a tiny tie-breaker once real media metrics exist.
    score += nameHint * 0.01;
...
function getQualityProbeSegments(streamUrl, playlist) {
    ...
    for (const rawLine of lines) {
        ...
        try {
            ...
            // A malformed entry simply cannot contribute to the quality probe.
        }
...
function parseH264Sps(nal) {
    try {
        ...
        const chromaArrayType = separateColourPlaneFlag
            ? 0
            : chromaFormatIdc;
        let cropUnitX = 1;
...
async function probeLogicalVariantQuality(req, group, variant) {
    ...
    try {
        ...
        const segments = getQualityProbeSegments(
            upstream.streamUrl,
            upstream.playlist
        );
        ...
        const result = {
            ...
            fps: media && Number.isFinite(media.fps)
                ? Number(media.fps.toFixed(3))
                : null,
        ...
        logicalVariantQuality.set(
            getLogicalQualityKey(group, variant),
            result
        );
...
async function getRankedLogicalVariants(req, group) {
    ...
    // Never switch away from a healthy active stream just for a better score.
...
const scoreSelfTests = [
  [{ width: 1920, height: 1080, fps: 50, bitrateKbps: 8000, stale: false, loopSuspect: false }, 'TEST FHD', 80],
  [{ width: 3840, height: 2160, fps: 25, bitrateKbps: 4000, stale: false, loopSuspect: false }, 'TEST 4K', 70],
  [{ width: 1920, height: 1080, fps: 50, bitrateKbps: 8000, stale: false, loopSuspect: true }, 'TEST FHD', -900]
];

for (const [result, name, minimum] of scoreSelfTests) {
  const nameHint = /4K/i.test(name) ? 4 : /FHD/i.test(name) ? 3 : 1;
  let score =
    (result.width >= 3800 ? 50 : result.width >= 1900 ? 40 : 20) +
    (result.fps >= 49 ? 25 : result.fps >= 29 ? 18 : 12) +
    (result.bitrateKbps >= 8000 ? 22 : result.bitrateKbps >= 3000 ? 14 : 6) +
    nameHint * 0.01;
  if (result.loopSuspect) score -= 1000;
  if (score < minimum) {
    throw new Error('logical quality score self-test failed for ' + name);
  }
}
