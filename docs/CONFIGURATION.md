...
For MPEG-TS (`.ts`) segments, the media-cache key is deliberately stable across renewed upstream query parameters: it is based on the segment origin and pathname rather than the signed query string. Two URLs that point to the same `segment_368.ts` with different renewed tokens therefore reuse the same prefetched payload instead of downloading the segment again. This normalization does not alter the upstream request URL or `/hls-proxy` HMAC validation, and playlists or other non-TS resources continue to use their full URL as the cache identity.
