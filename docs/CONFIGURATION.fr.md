...
| `VAVOO_HLS_ASSET_CACHE_TTL_SECONDS` | `120` | Durée du cache mémoire des ressources média HLS téléchargées avec succès. La valeur de 120 s laisse une marge suffisante aux segments préchargés avant leur consommation par un lecteur volontairement retardé derrière le direct. |
...
| `VAVOO_LOOP_QUARANTINE_SECONDS` | `86400` | Durée de quarantaine d’une variante lorsqu’une boucle vidéo est confirmée. Une journée évite qu’une source structurellement en boucle revienne polluer chaque failover tout en permettant de la retester le lendemain. |
...
Le tampon `VAVOO_HLS_LIVE_EDGE_DELAY_SEGMENTS` agit après le téléchargement de la playlist amont mais avant sa présentation à Kodi ou à tout autre consommateur du proxy. Avec la valeur par défaut `2`, les deux segments les plus récents restent cachés pendant qu’ils sont préchargés ; une playlist de six segments en expose donc généralement quatre. Lorsqu’un segment devient visible au poll suivant, il a normalement déjà été téléchargé et conservé dans le cache média. Le TTL de 120 secondes évite qu’un segment préchargé expire juste avant sa consommation lorsque le lecteur reste plusieurs segments derrière le direct. Les playlists VOD terminées (`#EXT-X-ENDLIST`) ne sont jamais retardées, et le proxy conserve toujours au moins trois segments visibles.

Pour les segments MPEG-TS (`.ts`), la clé du cache média est volontairement stable à travers le renouvellement des paramètres de l’URL amont : elle repose sur l’origine et le chemin du segment, sans la query string signée. Deux URL qui désignent par exemple le même `segment_368.ts` avec des jetons différents réutilisent donc le même payload déjà préchargé au lieu de le télécharger une seconde fois. Cette normalisation ne modifie pas l’URL réellement envoyée à l’amont ni la validation HMAC de `/hls-proxy`, et les playlists ou autres ressources non-TS conservent leur URL complète comme identité de cache.
